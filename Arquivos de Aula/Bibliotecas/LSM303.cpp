...
String LSM303::angulos(){
    float beta, alpha, gama;
    String resultado;

    beta  = atan2(-N.z,sqrt(N.x*N.x + N.y*N.y));
    alpha = atan2(N.y/cos(beta), N.x/cos(beta));
    gama  = atan2(E.z/cos(beta), Q.z/cos(beta));

    resultado = String(beta) + ":" + String(-gama) + ":" + String(alpha);

    return resultado;
  }
  
void LSM303::calibrar(LSM303::vector<int16_t> *m_min, LSM303::vector<int16_t> *m_max){
